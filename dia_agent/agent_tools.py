"""Controlled tools exposed to the ReAct reasoner.

The agent can decide *which* tool to call next, but each tool still runs
deterministic application logic. This keeps the medical safety boundary in
Python code instead of handing it to the model.
"""
...
@dataclass
class ToolExecution:
    """Result returned after executing one controlled tool."""
...
class AgentToolbox:
    """A small, controlled tool surface for the ReAct reasoner."""
    ...
    def seed_snippets(self, snippets: list[RagSnippet]) -> None:
        """Seed already-known snippets into the toolbox without re-querying."""
        self._remember_snippets(snippets)

    def describe_tools(self) -> str:
        """Human-readable tool list injected into the agent prompt."""
        return "\n".join(
            [
    ...
    def execute(self, action: str, action_input: dict[str, Any] | None = None) -> ToolExecution:
        """Execute one supported tool and record a lightweight audit log."""
            ...
            if tool_name == "get_patient_state":
                ...
                execution = ToolExecution(
                    ...
                    observation=(
                        ...
                        "retrieve_guidelines, list_safe_current_drugs, finish。"
                    ),
                )
